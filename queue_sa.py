...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        ...
        # if full it will create a new queue with the double the size
        if self._current_size == self._sa.size :
            self._double_queue()

        #Gettting the last nodes index
        self._back = (self._back + 1) % self._sa.size
        # Inserting the value and incrementing the
        self._sa[self._back] = value
        self._current_size += 1

    def dequeue(self) -> object:

        # Checking Weather the array has elements
        if self._current_size == 0:
            raise QueueException

        # getting the front node
        node = self._sa[self._front]
        # deleting the front node and reducing the size
        self._front = (self._front + 1) % self._sa.size
    ...
    def _double_queue(self) -> None:
        ...
        # This function will create a new Queue twice the lenght of given Queue
        # and it copy all the data to that
        front_index = self._front
        size = self._sa.size

        # calls the new queue class to create a double size Queue
        new_array = NewQueue(size*2)

        # coping the data from existing queue to new
        for _ in range(size):
            out = str(self._sa[front_index])
            new_array.enqueue(out)
            front_index = self._increment(front_index)
        # Making the new queue as default queue
        self.__dict__.update(new_array.__dict__)


# This is class extends from Queue class
# this class takes a size of queue to create new

class NewQueue(Queue):

    def __init__(self, size: int = 4) -> None:
        """Initialize new queue based on Static Array."""
        # default queue size was 4
        self._sa = StaticArray(size)
        self._front = 0
        self._back = -1
        self._current_size = 0
